...
RSpec.describe Internal::CheckNewVersionsJob do
  ...
  it 'updates the plan info' do
    data = { 'version' => '1.2.3', 'plan' => 'enterprise', 'plan_quantity' => 1, 'conviq_support_website_token' => '123',
             'conviq_support_identifier_hash' => '123', 'conviq_support_script_url' => '123' }
    ...
    expect(InstallationConfig.find_by(name: 'CONVIQ_SUPPORT_WEBSITE_TOKEN').value).to eq '123'
    expect(InstallationConfig.find_by(name: 'CONVIQ_SUPPORT_IDENTIFIER_HASH').value).to eq '123'
    expect(InstallationConfig.find_by(name: 'CONVIQ_SUPPORT_SCRIPT_URL').value).to eq '123'
  end
