  ...
  describe 'POST /resend_confirmation' do
    ...
    context 'when hCaptcha is configured' do
      before do
        ...
        allow(ConviqCaptcha).to receive(:new).and_return(captcha)
      end

      context 'with a valid captcha response' do
        let(:captcha) { instance_double(ConviqCaptcha, valid?: true) }
      ...
      context 'with an invalid captcha response' do
        let(:captcha) { instance_double(ConviqCaptcha, valid?: false) }
