...
describe Email::SendOnEmailService do
  ...
  describe '#perform' do
    ...
    context 'when an error occurs' do
      ...
      let(:exception_tracker) { instance_double(ConviqExceptionTracker, capture_exception: true) }
      ...
      before do
        ...
        allow(ConviqExceptionTracker).to receive(:new).and_return(exception_tracker)
      end

      it 'captures the exception' do
        expect(ConviqExceptionTracker).to receive(:new).with(error, account: message.account)
