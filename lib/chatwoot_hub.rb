...
class ConviqHub
  DEFAULT_BASE_URL = 'https://hub.2.conviq.com'.freeze
  ...
  def self.pricing_plan
    return 'community' unless ConviqApp.enterprise?

    InstallationConfig.find_by(name: 'INSTALLATION_PRICING_PLAN')&.value || 'community'
  end

  def self.pricing_plan_quantity
    return 0 unless ConviqApp.enterprise?
  ...
  def self.support_config
    {
      support_website_token: InstallationConfig.find_by(name: 'CONVIQ_SUPPORT_WEBSITE_TOKEN')&.value,
      support_script_url: InstallationConfig.find_by(name: 'CONVIQ_SUPPORT_SCRIPT_URL')&.value,
      support_identifier_hash: InstallationConfig.find_by(name: 'CONVIQ_SUPPORT_IDENTIFIER_HASH')&.value
    }
  end
  ...
  def self.instance_config
    {
      ...
      installation_version: Conviq.config[:version],
  ...
  def self.sync_with_hub
    begin
      ...
      ConviqExceptionTracker.new(e).capture_exception
    end
    parsed_response
  end
...
ConviqHub.singleton_class.prepend_mod_with('ConviqHub')
