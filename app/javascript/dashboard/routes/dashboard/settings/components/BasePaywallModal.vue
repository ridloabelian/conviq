...
<template>
  ...
  >
    ...
    <p class="text-sm font-normal text-n-slate-11">
      ...
      <span v-if="isOnConviqCloud && !isAdmin">
        {{ $t('GENERAL_SETTINGS.LIMIT_MESSAGES.NON_ADMIN') }}
      </span>
      <span v-else-if="!isOnConviqCloud && !isSuperAdmin">
        {{ $t(`${featurePrefix}.ENTERPRISE_PAYWALL.ASK_ADMIN`) }}
      </span>
    </p>
    <template v-if="isOnConviqCloud && isAdmin">
    ...
    <template v-else-if="!isOnConviqCloud && isSuperAdmin">
