...
const latestConviqVersion = computed(() => {
  return currentAccount.value.latest_conviq_version;
});

const globalConfig = useMapGetter('globalConfig/get');

const hasAnUpdateAvailable = computed(() => {
  if (!semver.valid(latestConviqVersion.value)) {
    return false;
  }

  return semver.lt(globalConfig.value.appVersion, latestConviqVersion.value);
});
...
<template>
  <div class="p-4 text-sm text-center">
    <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
      {{
        t('GENERAL_SETTINGS.UPDATE_CONVIQ', {
          latestConviqVersion: latestConviqVersion,
        })
      }}
    </div>
