...
const fetchSSLStatus = () => {
  if (!isOnConviqCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on Conviq cloud)
  if (portalObj?.custom_domain && isOnConviqCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
