...
export default {
  components: { Banner },
  props: {
    latestConviqVersion: { type: String, default: '' },
  },
  setup() {
  ...
  computed: {
    ...
    updateAvailable() {
      return hasAnUpdateAvailable(
        this.latestConviqVersion,
        this.globalConfig.appVersion
      );
    },
    bannerMessage() {
      return this.$t('GENERAL_SETTINGS.UPDATE_CONVIQ', {
        latestConviqVersion: this.latestConviqVersion,
      });
    },
    shouldShowBanner() {
      return (
        ...
        !this.isVersionNotificationDismissed(this.latestConviqVersion) &&
        this.isAdmin
      );
    },
  },
  methods: {
    ...
    dismissUpdateBanner() {
      ...
      if (updatedDismissedItems instanceof Array) {
        updatedDismissedItems.push(this.latestConviqVersion);
      } else {
        updatedDismissedItems = [this.latestConviqVersion];
      }
      LocalStorage.set(
...
<template>
  <Banner
    ...
    href-link="https://github.com/conviq/conviq/releases"
