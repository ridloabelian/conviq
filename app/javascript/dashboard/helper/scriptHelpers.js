...
export const initializeConviqEvents = () => {
  emitter.on(CONVIQ_RESET, () => {
    if (window.$conviq) {
      window.$conviq.reset();
    }
  });
  emitter.on(CONVIQ_SET_USER, ({ user }) => {
    if (window.$conviq) {
      window.$conviq.setUser(user.email, {
        avatar_url: user.avatar_url,
        email: user.email,
        identifier_hash: user.hmac_identifier,
        name: user.name,
      });
      window.$conviq.setCustomAttributes({
