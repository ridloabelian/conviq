class Twilio::TemplateProcessorService
  ...
  def convert_text_template(conviq_params)
    return process_key_value_params(conviq_params['processed_params']) if conviq_params['processed_params'].present?

    process_whatsapp_format_params(conviq_params['parameters'])
  end
  ...
  def convert_media_template(conviq_params)
    content_variables = {}

    # Handle processed_params format (key-value pairs)
    if conviq_params['processed_params'].present?
      conviq_params['processed_params'].each do |key, value|
        content_variables[key.to_s] = value.to_s
      end
    else
      # Handle parameters format (WhatsApp Cloud API format)
      parameter_index = 1
      conviq_params['parameters']&.each do |component|
