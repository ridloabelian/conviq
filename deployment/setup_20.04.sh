...
# Description: Install and manage a Conviq installation.
...
CONVIQ_HUB_URL="https://hub.2.conviq.com/events"
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/conviq-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Conviq (e.g., conviq.domain.com): ' domain_name
...
serve Conviq at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "conviq"; then
    adduser --disabled-password --gecos "" conviq
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/conviq/config
  file="/opt/conviq/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/conviq/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create conviq db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER conviq CREATEDB;
    ALTER USER conviq PASSWORD :'pass';
    ALTER ROLE conviq SUPERUSER;
...
function setup_conviq() {
  ...
  git clone https://github.com/conviq/conviq.git
  cd conviq
  git checkout "$BRANCH"
  bundle
  pnpm i
  ...
  sed -i -e '/POSTGRES_USERNAME/ s/=.*/=conviq/' .env
...
function run_db_migrations(){
  sudo -i -u conviq << EOF
  cd conviq
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:conviq_prepare
EOF
}

##############################################################################
# Setup Conviq systemd services and cwctl CLI
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function configure_systemd_services() {
  ...
  if [ -f "/etc/systemd/system/conviq.target" ]; then
  ...
  if [ "$DEPLOYMENT_TYPE" == "web" ]; then
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop conviq.target || true
      systemctl disable conviq.target || true
      systemctl stop conviq-worker.1.service || true
      systemctl disable conviq-worker.1.service || true
    fi

    # Stop and disable worker target if converting from worker-only
    if [ -f "/etc/systemd/system/conviq-worker.target" ]; then
      echo "Converting from worker-only to web-only"
      systemctl stop conviq-worker.target || true
      systemctl disable conviq-worker.target || true
    fi

    cp /home/conviq/conviq/deployment/conviq-web.1.service /etc/systemd/system/conviq-web.1.service
    cp /home/conviq/conviq/deployment/conviq-web.target /etc/systemd/system/conviq-web.target

    systemctl daemon-reload
    systemctl enable conviq-web.target
    systemctl start conviq-web.target
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop conviq.target || true
      systemctl disable conviq.target || true
      systemctl stop conviq-web.1.service || true
      systemctl disable conviq-web.1.service || true
    fi

    # Stop and disable web target if converting from web-only
    if [ -f "/etc/systemd/system/conviq-web.target" ]; then
      echo "Converting from web-only to worker-only"
      systemctl stop conviq-web.target || true
      systemctl disable conviq-web.target || true
    fi

    cp /home/conviq/conviq/deployment/conviq-worker.1.service /etc/systemd/system/conviq-worker.1.service
    cp /home/conviq/conviq/deployment/conviq-worker.target /etc/systemd/system/conviq-worker.target

    systemctl daemon-reload
    systemctl enable conviq-worker.target
    systemctl start conviq-worker.target
    ...
    cp /home/conviq/conviq/deployment/conviq-web.1.service /etc/systemd/system/conviq-web.1.service
    cp /home/conviq/conviq/deployment/conviq-worker.1.service /etc/systemd/system/conviq-worker.1.service
    cp /home/conviq/conviq/deployment/conviq.target /etc/systemd/system/conviq.target

    systemctl daemon-reload
    systemctl enable conviq.target
    systemctl start conviq.target
  fi

  cp /home/conviq/conviq/deployment/conviq /etc/sudoers.d/conviq
  cp /home/conviq/conviq/deployment/setup_20.04.sh /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/conviq/conviq/develop/deployment/nginx_conviq.conf
  cp nginx_conviq.conf /etc/nginx/sites-available/nginx_conviq.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/conviq.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_conviq.conf
  ln -s /etc/nginx/sites-available/nginx_conviq.conf /etc/nginx/sites-enabled/nginx_conviq.conf
...
EOF
  ...
  # Restart the appropriate conviq target
  if [ -f "/etc/systemd/system/conviq-web.target" ]; then
    systemctl restart conviq-web.target
  elif [ -f "/etc/systemd/system/conviq-worker.target" ]; then
    systemctl restart conviq-worker.target
  else
    systemctl restart conviq.target
  fi
}
...
function setup_logging() {
  touch /var/log/conviq-setup.log
  LOG_FILE="/var/log/conviq-setup.log"
}
...
***************************************************************************
Woot! Woot!! Conviq server installation is complete.
The server will be accessible at https://$domain_name

Join the community at https://conviq.com/community?utm_source=cwctl
***************************************************************************
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Conviq CLI tool to manage your installation.'
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.conviq.com/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
***************************************************************************
              Conviq Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/conviq-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Conviq?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Conviq. This takes a long while."
  setup_conviq &>> "${LOG_FILE}"
...
https://www.conviq.com/docs/deployment/deploy-conviq-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u conviq bash -c " cd conviq && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Conviq installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Conviq
  -I BRANCH                 Install Conviq from a git branch
  -u, --upgrade             Upgrade Conviq to the latest stable version
  -U BRANCH                 Upgrade Conviq from a git branch (EXPERIMENTAL)
...
Management:
  ...
  -l, --logs                View logs from Conviq. Supported values include web/worker.
  -r, --restart             Restart Conviq server
...
Report bugs at https://github.com/conviq/conviq/issues
Get help, https://conviq.com/community?utm_source=cwctl

EOF
}

##############################################################################
# Get Conviq web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u conviq-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u conviq-web.1.service -f
  fi
}
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Conviq v2.17+)"
...
##############################################################################
# Install pnpm - this replaces yarn starting from Conviq 4.0
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Conviq to v$CW_VERSION (branch: $BRANCH)"
...
EOF
  ...
  if [[ "$(printf '%s\n' "$CW_VERSION" "4.0" | sort -V | head -n 1)" == "4.0" ]]; then
    echo "Conviq v4.0 and above requires pgvector support in PostgreSQL."
  ...
  # Navigate to the Conviq directory
  cd conviq
...
##############################################################################
# Restart Conviq server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  if [ -f "/etc/systemd/system/conviq-web.target" ]; then
    systemctl restart conviq-web.target
    systemctl status conviq-web.target
  elif [ -f "/etc/systemd/system/conviq-worker.target" ]; then
    systemctl restart conviq-worker.target
    systemctl status conviq-worker.target
  else
    systemctl restart conviq.target
    systemctl status conviq.target
  fi
}

##############################################################################
# Convert existing Conviq deployment to different type (--convert)
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function convert_deployment() {
  echo "Converting Conviq deployment to: $DEPLOYMENT_TYPE"

  # Check if Conviq is installed
  if [ ! -d "/home/conviq/conviq" ]; then
    echo "Conviq installation not found. Use --install first."
    exit 1
  fi
...
function report_event() {
  ...
  curl -X POST -H "Content-Type: application/json" -d "$data" "$CONVIQ_HUB_URL" -s -o /dev/null
}
...
function get_installation_identifier() {
  ...
  installation_identifier=$(sudo -i -u conviq << "EOF"
  cd conviq
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/conviq/conviq/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://get.conviq.app/linux/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
